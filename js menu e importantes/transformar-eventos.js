...
// Con navegación por teclado usando flechas y Enter
(() => {
  ...
  function addStyles() {
    ...
    style.textContent = `
      ...
      .gtv-card{background:#1e1e1e;color:#fff;border-radius:16px;padding:18px 20px;box-shadow:0 10px 30px rgba(0,0,0,.45);font-family:system-ui,Segoe UI,Roboto,Arial,sans-serif;width:100%;box-sizing:border-box;min-height:140px;margin:12px 0; text-align:center; transition: all 0.2s ease}
      ...
      .gtv-card:focus{outline:none;transform:scale(1.02);box-shadow:0 15px 40px rgba(0,0,0,.6)}
      .gtv-card.gtv-focused{outline:none;transform:scale(1.02);box-shadow:0 15px 40px rgba(0,0,0,.6)}
      .gtv-status{display:flex;align-items:center;justify-content:center;gap:6px;margin-bottom:8px;font-size:12px;font-weight:600}
      .gtv-dot{width:8px;height:8px;border-radius:50%;display:inline-block}
      .gtv-dot.live{background:#ff4444;animation:pulse 2s infinite}
      .gtv-dot.finished{background:#4caf50}
  ...
  function updateFocusableCards() {
    focusableCards = Array.from(document.querySelectorAll('#events-container .gtv-card.gtv-click'));
    currentFocusIndex = Math.max(-1, Math.min(currentFocusIndex, focusableCards.length - 1));
  }

  function focusCard(index) {
    // Remover focus de todas las tarjetas
    focusableCards.forEach(card => {
      card.classList.remove('gtv-focused');
      card.blur();
    });
    
    if (index >= 0 && index < focusableCards.length) {
      currentFocusIndex = index;
      const card = focusableCards[index];
      card.classList.add('gtv-focused');
      card.focus();
      
      // Scroll suave hacia la tarjeta enfocada
      card.scrollIntoView({ 
        behavior: 'smooth', 
        block: 'center',
        inline: 'nearest'
  ...
  function handleKeyNavigation(e) {
    ...
    switch(e.key) {
      ...
      case 'ArrowRight':
        e.preventDefault();
        const nextIndex = currentFocusIndex < focusableCards.length - 1 ? currentFocusIndex + 1 : 0;
        focusCard(nextIndex);
        break;
      ...
      case 'ArrowLeft':
        e.preventDefault();
        const prevIndex = currentFocusIndex > 0 ? currentFocusIndex - 1 : focusableCards.length - 1;
        focusCard(prevIndex);
        break;
        
      case 'Enter':
      case ' ':
        if (currentFocusIndex >= 0 && currentFocusIndex < focusableCards.length) {
          e.preventDefault();
          const card = focusableCards[currentFocusIndex];
      ...
      case 'End':
        e.preventDefault();
        focusCard(focusableCards.length - 1);
        break;
    }
  }
  ...
  function start() {
    ...
    const obs = new MutationObserver(() => {
      setTimeout(() => {
        transformAll();
        // Mantener el focus en la tarjeta actual después de transformaciones
    ...
    window.__gtvCardsStop = () => {
      obs.disconnect();
      document.removeEventListener('keydown', handleKeyNavigation);
    };
    
    // Función para enfocar la primera tarjeta
    window.__gtvCardsFocusFirst = () => {
    ...
    window.__gtvCardsInfo = () => {
      return {
        totalCards: focusableCards.length,
        currentIndex: currentFocusIndex,
        currentCard: currentFocusIndex >= 0 ? focusableCards[currentFocusIndex] : null
      };
    };
  }
